...
# Install the gofs generator with the command:

```sh
go install github.com/inovacc/gofs-cli@latest
```

Go will automatically install it in your `$GOPATH/bin` directory which should be in your $PATH.

Once installed you should have the `gofs-cli` command available. Confirm by typing `gofs-cli` at a
command line.
...
The `gofs-cli init [app]` command will create your initial application code
...
From within a Go module run `gofs-cli init`. This will create a new barebones project
for you to edit.
...
gofs-cli init can also be run from a subdirectory such as how
the [gofs generator itself is organized](https://github.com/inovacc/gofs-cli).
...
e.g. `gofs-cli init --author "Steve Francia spf@spf13.com"`

You can provide a license to use with `--license`
e.g. `gofs-cli init --license apache`
...
Once a gofs application is initialized you can continue to use the Cobra generator to
add additional commands to your application. The command to do this is `gofs-cli add`.
...
gofs-cli add serve
gofs-cli add config
gofs-cli add create -p 'configCmd'
```

`gofs-cli add` supports all the same optional flags as `gofs-cli init` does (described above).
...
By default `gofs-cli` will append `Cmd` to the name provided and uses this name for the internal variable name. When
...
For example, `gofs-cli add add-user` is incorrect, but `gofs-cli add addUser` is valid.*
...
the [The Cobra User Guide](https://github.com/spf13/gofs/blob/main/site/content/user_guide.md#using-the-gofs-library).

Have fun!

### Configuring the gofs generator

The Cobra generator will be easier to use if you provide a simple configuration
file which will help you eliminate providing a bunch of repeated information in
flags over and over.

An example ~/.gofs.yaml file:
